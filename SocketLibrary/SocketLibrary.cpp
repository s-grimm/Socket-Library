...
SocketLibrary::SocketLibrary( USHORT port, IPPROTO protocol, USHORT socketType ) :	_port(port), _protocol(protocol), _socketType(socketType), _addressFamily(AF_INET) {}
SocketLibrary::SocketLibrary( USHORT port, IPPROTO protocol, ADDRESS_FAMILY addressFamily ) :	_port(port), _protocol(protocol), _socketType(SOCK_DGRAM), _addressFamily(addressFamily) {}
